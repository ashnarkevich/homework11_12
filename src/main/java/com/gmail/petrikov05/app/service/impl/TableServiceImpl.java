...
import com.gmail.petrikov05.app.service.constant.AddRoleActionEnum;
import com.gmail.petrikov05.app.service.constant.AddUserActionEnum;
...
public class TableServiceImpl implements TableService {
    ...
    public void addRole() {
        try (Connection connection = connectionRepository.getConnection()) {
            connection.setAutoCommit(false);
            try {
                for (AddRoleActionEnum action : AddRoleActionEnum.values()) {
                    tableRepository.executeQuery(connection, action.getQuery());
                }
                connection.commit();
            } catch (SQLException e) {
    ...
    public void addUser() {
        try (Connection connection = connectionRepository.getConnection()) {
            connection.setAutoCommit(false);
            try {
                for (AddUserActionEnum action : AddUserActionEnum.values()) {
                    tableRepository.executeQuery(connection, action.getQuery());
                }
                connection.commit();
            } catch (SQLException e) {
